...
- (void)setShowPercentage:(BOOL)showPercentage
{
    ...
    for(SliceLayer *layer in _pieView.layer.sublayers)
    {
        ...
        if(_showPercentage)
            ...
            label = (layer.text)?layer.text:[NSString stringWithFormat:@"%0.2f%%", layer.value];
    ...
    if (_dataSource)
    {
        ...
        */
        ...
        for(int index = 0; index < sliceCount; index ++)
        {
            ...
            [self updateLabelForLayer:layer value:[self.DLDataArray[index] doubleValue] index:index];
            [layer createArcAnimationForKey:@"startAngle"
...
- (NSInteger)getCurrentSelectedOnTouch:(CGPoint)point
{
    ...
    [pieLayers enumerateObjectsUsingBlock:^(id obj, NSUInteger idx, BOOL *stop) {
        ...
        if (CGPathContainsPoint(path, &transform, point, 0)) {
            ...
            [pieLayer setStrokeColor:[UIColor colorWithWhite:0.90 alpha:1].CGColor];
...
- (void)updateLabelForLayer:(SliceLayer *)pieLayer value:(CGFloat)value index:(int)index
{
    ...
    NSString *label;
    if(_showPercentage)
        label = [NSString stringWithFormat:format, pieLayer.percentage*100];
    else
        label = (pieLayer.text)?pieLayer.text:[NSString stringWithFormat:format, value];
...
- (void)renderInLayer:(DLPieChart *)layerHostingView dataArray:(NSMutableArray*)dataArray
{
    ...
    [self.DLColorsArray addObject:[UIColor colorWithRed:255.0 green:0.0 blue:0.0 alpha:1.0]];
    [self.DLColorsArray addObject:[UIColor colorWithRed:0.0 green:0.0 blue:255.0 alpha:1.0]];
    ...
    [layerHostingView setLabelFont:[UIFont systemFontOfSize:(24-DLDataArray.count/2)]];
    [layerHostingView setShowPercentage:NO];
    [layerHostingView setPieBackgroundColor:[UIColor colorWithWhite:0.80 alpha:1]];
