...
-(instancetype)initWithScene:(GameScene*) gameScene
{
...
//    int winner = [self winner];
    Hand *hand = [Hand new];
    hand.playerCards = self.currentPlayerCards;
    hand.bankerCards = self.currentBankerCards;
    hand.winner = [self winner];
    [self.hands addObject:hand];
    
    scene.myLabel.text = [self winnerMessage];
    scene.lblScore.text = [NSString stringWithFormat:@"P - %d : %d - B", [self playerTotal], [self bankerTotal]];
}
